...
class SupportRepository implements SupportInterface
{
    ...
    {
        return $this->model::orderBy($orderBy, $sortBy)->where('user_id', Auth::user()->id)->paginate(settings('paginate_value'));
    }
    public function get($id)
    {
        return $this->model::with('supportChats')->findOrFail($id);
    }
    ...
    public function store($request)
    {
        try {
            ...
            $support->date              = $request->date ?? now()->toDateString();
    ...
    public function reply($request)
    {
        try {
            ...
            $reply->attached_file     = $this->upload->uploadImage($request->attached_file, 'support/', [], $reply->attached_file);
            $reply->save();
            DB::commit();
            ...
            return $this->responseWithSuccess(__('alert.reply_msg'), []);
        } catch (\Throwable $th) {
            DB::rollBack();
            return $this->responseWithError(___('alert.something_went_wrong'), []);
        }
