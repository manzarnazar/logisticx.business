...
class FraudRepository implements FraudInterface
{
    public function all()
    {
        return Fraud::orderByDesc('id')->paginate(settings('paginate_value'));
    }

    public function filter()
    {
        return Fraud::where('created_by', Auth::user()->id)->orderByDesc('id')->paginate(settings('paginate_value'));
    }

    public function check($request)
    {
        return Fraud::where('phone', 'LIKE', '%' . $request->phone . '%')->orderByDesc('id')->paginate(settings('paginate_value'));
    }
