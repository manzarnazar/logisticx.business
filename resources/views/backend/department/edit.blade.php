...
@section('title')
{{ ___('common.department') }} {{ ___('label.edit') }}
@endsection
@section('maincontent')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="form-input-header">
                        <h4 class="title-site">{{ ___('label.edit') }} {{ ___('label.department') }}</h4>
                    </div>
                    <form action="{{route('departments.update',['id'=>$department->id])}}" method="POST" enctype="multipart/form-data" id="basicform">
                        ...
                        <div class="row">
                            <div class="form-group col-md-12">
                                ...
                                <input id="title" type="text" name="title" data-parsley-trigger="change" placeholder="{{ ___('placeholder.enter_title') }}" autocomplete="off" class="form-control input-style-1" value="{{old('title',$department->title)}}" require>
                                @error('title')
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <select name="status" class="form-control input-style-1 select2">
                                    ...
                                    <option value="{{ $key }}" {{ (old('status',$department->status) == $key) ? 'selected' : '' }}>{{ ___('common.'.  $status) }}</option>
                                    @endforeach
                                </select>
                                @error('status')
                                <small class="text-danger mt-2">{{ $message }}</small>
