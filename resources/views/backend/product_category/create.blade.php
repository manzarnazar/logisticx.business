...
@section('title')
{{ ___('charges.product_category') }} {{ ___('label.create') }}
@endsection
@section('maincontent')
<!-- wrapper  -->
<div class="container-fluid  dashboard-content">
    ...
    <div class="card">
        <div class="form-input-header">
            <h4 class="title-site"> {{ ___('label.create') }} {{ ___('charges.product_category') }} </h4>
        </div>

        <div class="card-body">
            <form action="{{route('productCategory.store')}}" method="POST" enctype="multipart/form-data" id="basicform">
                @csrf
                <div class="form-row">

                    <div class="form-group col-md-6">
                        <label class="label-style-1" for="name">{{___('label.name')}}</label> <span class="text-danger small">*</span>
                        <input class="form-control input-style-1 " id="name" type="text" name="name" value="{{old('name')}}" placeholder="{{ ___('placeholder.product_category') }}" autocomplete="off">
