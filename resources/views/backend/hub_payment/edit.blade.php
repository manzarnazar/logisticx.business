    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="form-input-header">
                <h4 class="title-site"> {{ ___('hub.edit_payment') }}</h4>
            </div>

            <form action="{{route('hub.hub-payment.update')}}" method="POST" enctype="multipart/form-data" id="hubPaymentForm">
                @csrf
                @method('PUT')
                ...
                <div class="form-row">
                    <div class="col-lg-6 col-12 form-group">
                        <label class="label-style-1" for="hub_id">{{ ___('hub.title') }}<span class="text-danger">*</span> </label>
                        <select id="hub_id" class="form-control input-style-1 select2" name="hub_id">
                            ...
                            @foreach ($hubs as $hub)
                            <option value="{{ $hub->id }}" @selected(old('hub_id',@$payment->hub_id)== $hub->id)>{{ $hub->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select id="hub_account_id" name="hub_account_id" class="form-control input-style-1 select2" data-url="{{ route('accounts.searchByHub') }}">
                            ...
                            @if(@$payment->hub_id)
                            @foreach ($payment->hub->accounts as $account)
                            <option value="{{ $account->id }}" @selected(old('hub_account_id',@$payment->hub_account_id) == $account->id) > {{ $account->text }}</option>
                            @endforeach
                            @endif
                    ...
                    <div class="col-lg-6 col-12 form-group">
                        ...
                        <input type="number" step="any" name="amount" id="amount" class="form-control input-style-1" value="{{old('amount',@$payment->amount)}}" placeholder="{{ ___('placeholder.enter_Amount') }}">
                    ...
                    <div class="col-12 col-md-6  form-group process">
                        ...
                        <input id="transaction_id" type="text" name="transaction_id" placeholder="{{ ___('placeholder.enter_Transaction_ID') }}" class="form-control input-style-1" value="{{old('transaction_id')}}">
                    ...
                    <div class="col-12 col-md-6 form-group">
                        ...
                        <textarea name="description" id="description" class="form-control input-style-1" placeholder="{{ ___('placeholder.enter_description')}}">{{ old('description',@$payment->description) }}</textarea>
