...
<div class="container-fluid  dashboard-content">
    ...
    <div class="card">
        <div class="form-input-header">
            <h4 class="title-site"> {{ ___('label.update')}} {{ ___('charges.vas') }}</h4>
        </div>
        <div class="card-body">
            <form action="{{route('vas.update')}}" method="POST" enctype="multipart/form-data" id="basicform">
                @csrf
                @method('put')

                <input type="hidden" name="id" value="{{$vas->id}}">

                <div class="form-row">
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            <label class="label-style-1" for="name">{{___('label.name')}}</label> <span class="text-danger small">*</span>
                            <input class="form-control input-style-1" id="name" type="text" name="name" value="{{old('name',$vas->name) }}" placeholder="Enter VAS name" autocomplete="off">
                            @error('name') <span class="text-danger small"> {{$message}}</span> @enderror
                        </div>
                    </div>
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            <label class="label-style-1" for="price">{{___('label.Price')}}</label>
                            <input type="number" step="any" class="form-control input-style-1" id="price" name="price" value="{{ old('price', $vas->price) }}" placeholder="Enter Price" autocomplete="off">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control input-style-1" id="position" name="position" value="{{ old('position',$vas->position) }}" placeholder="Enter Position" autocomplete="off">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-select input-style-1 select2" name="status" id="status">
                            ...
                            <option value="0" @if( old('status',$vas->status) == 0 ) selected @endif>Inactive</option>
                        </select>
