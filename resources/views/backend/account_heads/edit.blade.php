...
@section('title')
{{ ___('account.account_heads') }} {{ ___('label.edit') }}
@endsection
@section('maincontent')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="form-input-header">
                        <h4 class="title-site">{{___('label.edit')}} {{ ___('account.account_head') }}</h4>
                    </div>
                    <form action="{{route('account.heads.update',$account_heads->id)}}" method="POST" enctype="multipart/form-data" id="basicform">
                        @csrf
                        @method('PUT')

                        <input type="hidden" name="id" value="{{ $account_heads->id }}">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <select id="type" name="type" class="form-control input-style-1 select2">
                                    ...
                                    <option value="{{ $key }}" @selected(old('type',$account_heads->type) == $key) >{{ ___('account.'.$value)}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control input-style-1" id="name" name="name" value="{{ old('name', $account_heads->name) }}" placeholder="{{ ___('placeholder.enter_name')}}">
                            ...
                            <div class="form-group col-md-6 d-block" id="accountStatus">
                                ...
                                <select name="status" class="form-control input-style-1 select2">
                                    ...
                                    <option value="{{ $key }}" @selected(old('status', $account_heads->status) == $key) >{{ ___('common.'.  $status) }}</option>
                                    @endforeach
                                </select>
