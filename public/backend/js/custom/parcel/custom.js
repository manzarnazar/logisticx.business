...
$(document).ready(function () {
    ...
    $("#received_track_id").on("keyup", function () {
        ...
        if (response == false) {
            ...
            $.ajax({
                ...
                data: {
                    tracking_id: $("#received_track_id").val(),
                    transfer_hub_id: $("#received_transfer-hub").val(),
                },
                dataType: "json",
                success: function (data) {
                    response = false;
                    if (data == 0) {
                        ...
                        <tr>
                            ...
                            <td>${data.parcel_transaction.total_charge}</td>
                            <td>${data.parcel_transaction.cash_collection}</td>
    ...
    $("#delivery_man_assign_track_id").on("keyup", function () {
        ...
        if (_submit == 0) {
            ...
            $.ajax({
                type: "post",
                url: $("#delivery_man_assign_track_id").data("url"),
                ...
                success: function (data) {
                    ...
                    setTimeout(function () {
                        ...
                        if (data == 0) {
                            ...
                            const row = `
                            <tr> 
                                <td>${_sl++}<input type="hidden" value="${
                                data.id
                            }" name="parcel_ids_[]"></td>
                                <td>${data.tracking_id}</td>
                                <td>${data.merchant.business_name}</td>
                                ...
                                <td>${data.parcel_transaction.total_charge}</td>
                                <td>${
                                    data.parcel_transaction.cash_collection
                                }</td>
    ...
    document.addEventListener("scan", function (sScancode, data) {
        if (
            ...
            if ($("#delivery_man_assign_track_id").val() != "") {
                ...
                $.ajax({
                    type: "post",
                    url: $("#transfer_to_hub_track_id").data("url"),
                    data: {
    ...
    $("#transfer_to_hub_track_id").on("input", function () {
        $(".search_message").empty();
        const tracking_id = $(this).val();
        const hub_id = $("#transfer_hub").val();

        if (tracking_id == "") {
            return;
        }

        if (tracking_id.length > 20) {
            var message = `<small class="text-danger">Maximum 20 characters!</small>`;
            $(".search_message").html(message);
            return;
        }

        // multiple request handel
        if (searchedItems.includes(tracking_id)) {
            var message = `<small class="text-danger">Already searched .</small>`;
            $(".search_message").html(message);
            return;
        }

        searchedItems.push(tracking_id);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $(".removerow").click(function () {
                    $(this).parent().parent().remove();
                    --sl;
                    const index = searchedItems.indexOf(tracking_id);
                    if (index !== -1) {
        ...
        //  submit button disabled true/false
        if (hub_id != null) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data == 0) {
                        ...
                        var row = `<tr>
                                    <td>${a++}<input type='hidden' name='parcel_id[]' value='${
                            data.id
                        }'/></td>
                                    <td>${data.tracking_id}</td>
                                    <td>${data.merchant.business_name}</td>
                                    <td>${data.customer_phone}</td>
                                    <td>${
                                        data.parcel_transaction.total_charge
                                    }</td>
                                    <td>${
                                        data.parcel_transaction.cash_collection
                                    }</td>
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data == 0) {
                        ...
                        const row = `
                        <tr>
                            <td>${a++}<input type='hidden' name='parcel_ids[]' value='${
                            data.id
                        }'/></td>
                            <td>${data.tracking_id}</td>
                            <td>${data.merchant.business_name}</td>
