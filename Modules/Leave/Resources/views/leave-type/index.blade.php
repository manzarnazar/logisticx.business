...
@section('title',___('hr_manage.leave_type') )
@section('maincontent')
<!-- wrapper  -->
    ...
    <div class="j-parcel-main j-parcel-res">
        <div class="card">
            ...
            <div class="card-header mb-3">
                <h4 class="title-site">{{ ___('hr_manage.leave_type') }} {{ ___('label.list') }}</h4>
                @if (hasPermission('leave_type_create'))
                <a href="{{ route('leave.type.create') }}" class="j-td-btn">
            ...
            <div class="card-body">
                    ...
                    <table class="table">
                        <thead class="bg">
                            <tr>
                                ...
                                @if(hasPermission('leave_type_update') || hasPermission('leave_type_delete'))
                                <th scope="col">{{ ___('label.action') }}</th>
                                @endif
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($leave_types as $key=>$type )

                            <tr id="row_{{ $type->id }}">
                                <td>{{ ++$key }}</td>
                                <td>{{ @$type->name }}</td>
                                <td>{!! $type->position !!}</td>
                                <td>{!! $type->my_status !!}</td>
                                ...
                                <td>
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <a href="{{ route('leave.type.edit',$type->id) }}" class="dropdown-item"><i class="fa fa-edit" aria-hidden="true"></i> {{ ___('label.edit') }}</a>
                                        @endif
                                        @if(hasPermission('leave_type_delete') )
                                        <a class="dropdown-item" href="javascript:void(0);" onclick="delete_row('admin/leave/type/delete', {{$type->id}})"> <i class="fa fa-trash" aria-hidden="true"></i> {{ ___('label.delete') }} </a>
                                        @endif
                                    </div>
                                </td>
                                @endif
                ...
                <x-paginate-show :items="$leave_types" />
                @endif
