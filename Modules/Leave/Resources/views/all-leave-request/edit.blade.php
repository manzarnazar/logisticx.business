...
@section('title')
{{ ___('hr_manage.all_leave_request') }} {{ ___('label.edit') }}
@endsection
                ...
                <div class="col-12">
                    <div class="form-inputs">
                        <div class="form-input-header">
                            <h4 class="title-site">{{ ___('hr_manage.all_leave_request') }} {{ ___('label.edit') }}
                            </h4>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                <label class="label-style-1" for="userID">{{ ___('label.user') }}</label>
                                <select id="userID" name="user" class="form-control input-style-1 select2" data-url="{{ route('user.search') }}" data-placeholder="{{___('placeholder.select_user')}}">
                                    <option></option>
                                    <option selected value="{{$leave_request->user_id }}">{{$leave_request->user->name }}</option>
                                </select>
                                @error('user') <small class="text-danger mt-2">{{ $message }}</small> @enderror
                            </div>

                            <div class="form-group col-md-6">
                                <label class=" label-style-1" for="type_id">{{ ___('hr_manage.leave_type') }}<span class="text-danger">*</span> </label>
                                <select class="form-control input-style-1 select2" name="type_id" id="type_id" data-url="{{route('leave.request.types')}}" required>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="date" name="date" id="date" class="form-control input-style-1 flatpickr-range" placeholder="{{ ___('placeholder.select_date') }}" value="{{ $leave_request->from_date . ' to ' . $leave_request->to_date }}">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input id="reason" type="text" name="reason" placeholder="{{ ___('placeholder.reason') }}" class="form-control input-style-1" value="{{ old('reason',$leave_request->reason) }}" require>
