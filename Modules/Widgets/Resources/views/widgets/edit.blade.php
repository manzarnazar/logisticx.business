...
<div class="container-fluid  dashboard-content">
    ...
    <div class="j-create-main">
        <form action="{{ route('widgets.update',['id'=>$widget->id]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="row">
                <div class="col-12">
                    <div class="form-inputs">
                        <div class="form-input-header">
                            <h4 class="title-site">{{ ___('label.edit') }} {{ ___('label.widget') }}
                            </h4>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-12 col-lg-6">
                                <div class="form-group ">
                                    ...
                                    <input type="text" placeholder="{{ ___('label.enter_title') }}" class="form-control input-style-1" name="title" value="{{ old('title',$widget->title) }}">
                            ...
                            <div class="form-group col-12 col-lg-6">
                                ...
                                <select name="section" class="form-control input-style-1 select2" onchange="showImage()">
                                    ...
                                    <option value="{{ $value }}" @if(old('section',$widget->section) == $key) selected @endif >{{ ___("label.$key") }}</option>
                                    @endforeach
                            ...
                            <div class="form-group col-md-6">
                                <label class="label-style-1">{{ ___('label.background') }}</label>
                                <div class="custom-control custom-checkbox d-flex gap-6">
                                    <div class="form-check form-check-inline">
                                        <input type="radio" class="custom-control-input" id="none" name="background" value="none" {{$widget->background == 'none' ? 'checked' : ''}}>
                                        <label class="custom-control-label" for="none">{{ ___('label.none') }}</label>
                                    </div>
                                    <div class="form-check form-check-inline ml-5">

                                        <input type="radio" class="custom-control-input" id="bg_color" name="background" value="bg_color" {{$widget->background == 'bg_color' ? 'checked' : ''}}>
                                        <label class="custom-control-label" for="bg_color">{{ ___('label.bg_color') }}</label>
                                    </div>
                                    <div class="form-check form-check-inline ml-5">

                                        <input type="radio" class="custom-control-input" id="bg_image" name="background" value="bg_image" {{$widget->background == 'bg_image' ? 'checked' : ''}}>
                                        <label class="custom-control-label" for="bg_image">{{ ___('label.bg_image') }}</label>
                                    </div>
                            ...
                            <div class="col-lg-6 _bg_color {{$widget->background == 'bg_color' ? '' : 'd-none'}}" id="_bg_color">
                                <div class="form-group ">
                                    <label class="label-style-1">{{ ___('label.bg_color') }}</label>
                                    <input type="color" placeholder="{{ ___('placeholder.bg_color') }}" class="form-control input-style-1" name="bg_color" value="{{ old('bg_color', $widget->bg_color) }}">
                                    @error('bg_color') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="col-md-6 _bg_image {{$widget->background == 'bg_image' ? '' : 'd-none'}}" id="_bg_image">
                                <label class="label-style-1">{{ ___('label.bg_image') }}<span class="fillable"></span></label>
                                <div class="ot_fileUploader left-side mb-3">
                                    <img src="{{getImage($widget->upload)}}" alt=" " class="rounded">
                            ...
                            <div class="form-group col-12 col-lg-6">
                                ...
                                <select name="section_padding[]" class="form-control input-style-1 select2" multiple>
                                    @foreach (App\Enums\SectionPadding::cases() as $sectionPadding)
                                        <option value="{{ $sectionPadding->value }}" @selected(in_array($sectionPadding->value, old('section_padding', $widget->section_padding ?? [])))>
                            ...
                            <div class="col-12 col-lg-6">
                                <div class="form-group ">
                                    ...
                                    <input type="number" placeholder="{{ ___('placeholder.position') }}" class="form-control input-style-1" name="position" value="{{ old('position',$widget->position) }}">
                            ...
                            <div class="form-group col-12 col-lg-6">
                                ...
                                <select name="status" class="form-control input-style-1 select2">
                                    ...
                                    <option value="{{ $key }}" @selected(old('status', $widget->status) == $key)>{{ ___('common.'.  $status) }}</option>
                                    @endforeach
                                </select>
